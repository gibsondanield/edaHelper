...
class Unsupervised(object):
    ...
    def reduce_dimensions(self,ndim=3,methods=['SVD'], classifier=None, plot2d=True, plot3d=False):
        self.ndim=ndim
        ...
        if 'SVD' in methods:
            ...
            self.tData = self.tSVD.fit_transform(self.X)
            if plot2d:
                scatter2d.scatter(self.tData.T[0],self.tData.T[1])                
                
            if plot3d:
                scatter3d.scatterBinary(self.tData,self.y, clf.predict(self.X))
...
#             print type(self.X)
             plt.plot(kde_statsmodels_m(self.features_df[self.y][v],np.linspace(0,12,2000),bandwidth=bandwidth),label='True')
             plt.plot(kde_statsmodels_m(self.features_df[map(lambda x:not x,self.y)][v],np.linspace(0,12,2000),bandwidth=bandwidth),label='False')
        plt.legend(loc=0)
        plt.tight_layout()
    ...
    def plot3D_predictions(self,features_to_use='all',max_plots=3, max_n_features=9, models=['SVC', 'GaussianNB'],plot_decision_boundary=False):
        ...
        scatter3d.scatterBinary(self.features_df,self.y, self.rf.predict(self.X))
...
if __name__ == '__main__':
    ...
    X, y = make_classification(n_features=10,n_samples=1000,n_informative=5,n_clusters_per_class=3,n_redundant=0,hypercube=True,flip_y=.5)
    X=pd.DataFrame(X)
